...
class CalendarController extends AppController {
    ...
    public function index() {
        ...
        $aryUser = $this->Session->read('Auth.User');
        //debug($aryUser);
        if ($aryUser['authority_id'] == 1)
            $condition1 = array('Audit.audit_create_user' => $aryUser['user_id']);
        if ($aryUser['authority_id'] == 2)
            $condition1 = array('Audit.status' => 2);
        if (isset($search)) {
            ...
            if ($search['searchForm']['item_id'] != '0')
                $condition += array('Audit.item_id' => $search['searchForm']['item_id']);
            if ($search['searchForm']['user_id'] != '0')
                $condition += array('Audit.audit_create_user' => $search['searchForm']['user_id']);
            if ($search['searchForm']['status_n'] != '')
                $condition += array('Audit.status' => $search['searchForm']['status_n']);
            //debug($condition);die();
        }
        else {
            ...
            if ($aryUser['authority_id'] == 1)
                $search['searchForm']['user_id'] = $aryUser['user_id'];
            else
                $search['searchForm']['user_id'] = '0';
            if ($aryUser['authority_id'] == 2)
                $search['searchForm']['status_n'] = 2;
            else
                $search['searchForm']['status_n'] = '';
            $start_f = 1;
        }
    ...
    public function register($dates = null) {
        ...
        if ($this->request->is('post')) {
            $data = $this->request->data;
            //debug($data);
            if ($data['Calendar']['store_id'] == 0)
                $data['Calendar']['store_id'] = '';
