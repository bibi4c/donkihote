...
class Uploader {
  ...
  function __handleUnique($target_path){
	//$target_path = 'audit_'.$target_path;
  ...
  function processFile($file = null){
	...
	//$this->Session->write('fileabc','111111111');
	
	$filebibi = $this->file['size'].$this->_ext();
    $fileName = $filebibi;
    ...
    //now move the file.
	// $filename = iconv('UTF-8','ISO-8859-1//TRANSLIT',$this->file['tmp_name']);
