...
<div class="mainarea">
    ...
    <table id="summaryField">
        <col width="30%" />
        <col width="70%" />
        <?php echo $this->Form->create('Document', array('url' => array('controller' => 'Document', 'action' => 'register_confirm'))); ?>
        <tr>
            <th>ジャンル</th>
            <td>
                <?php
                echo $items['0']['Item']['name'];
                echo $this->Form->hidden('item', array('value' => $items['0']['Item']['item_id'], 'label' => false));
                ?>
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                echo $this->Form->hidden('category', array('value' => $category['0']['Category']['category_id'], 'label' => false));
                ?>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <?php
                if (strlen($document_data['Document']['start_date']) > 0) {
                    $date = $document_data['Document']['start_date'];
        ...
        <tr>
            ...
            <td>
                <?php
                if (strlen($document_data['Document']['end_date']) > 0) {
                    $date = $document_data['Document']['end_date'];
                    $date = date('Y年n月j日', strtotime($date));
                } else
                    $date = '';
                echo $date;
...
<?php echo $this->Html->link('キャンセル', array('controller' => 'Document', 'action' => 'register'), array('class' => 'btnCanceller linkCanceller posLeft')); ?>
